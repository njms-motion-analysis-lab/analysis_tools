...
class Predictor(LegacyBaseModel):
    ...
    def get_final_bdf(self, force_load=False, force_abs_x=False, untrimmed=False, get_sg_count=False, add_other=False):
        ...
        valid_indices = (bdf['patient'] != -1) & (bdf['patient'].notna())
...
# Remove this after new data (add_other)
    ...
    def extract_last_two_columns(self, bdf, compare_across_cohort = False):
        ...
        if all(col in bdf.columns for col in columns_to_extract):
            self.last_two_columns = bdf[columns_to_extract].copy()
            # Note: No need to drop 'cohort' anymore, as its values have been replaced
            shortened_bdf = bdf.drop(columns=[columns_to_extract[1]])  # Dropping only 'str_patient'
        else:
            self.last_two_columns = pd.DataFrame(columns=columns_to_extract)
            shortened_bdf = bdf.copy()

        return shortened_bdf, self.last_two_columns 
    ...
    def clean_bdf(self, bdf):
        ...
        bdf['patient'].fillna(-1, inplace=True)

        # Reorder columns to move 'str_patient' in front of 'is_dominant', if necessary
        col_order = ['str_patient'] + [col for col in bdf.columns if col != 'str_patient']
        bdf = bdf[col_order]

        # Replace infinities and handle large values, as in the original function
        bdf.replace([np.inf, -np.inf], np.nan, inplace=True)
        bdf.fillna(0, inplace=True)  # Replace NaNs with 0s after the above replacement
        float32_max = np.finfo(np.float32).max
        float32_min = np.finfo(np.float32).min
        # Identify numeric columns in the DataFrame
        numeric_cols = bdf.select_dtypes(include=[np.number]).columns

        # Clip values only in numeric columns to the float32 range
        bdf[numeric_cols] = bdf[numeric_cols].clip(lower=np.finfo(np.float32).min, upper=np.finfo(np.float32).max)
    ...
    def fit_multi_models(self, bdf, y, use_shap=True):
        print(bdf)
    ...
    def get_predictor_scores(self, score_type=None, model_name=None):
        ...
        if score_type is not None and model_name is not None:
            ...
            if self.multi_predictor_id is not None:
                scores = PredictorScore.where(
            ...
            if self.multi_predictor_id is not None:
                scores = PredictorScore.where(predictor_id=self.id, multi_predictor_id=self.multi_predictor_id)    
                if len(scores) is 0:
    ...
    def get_fold_corr(self, model_name=None):
        
        pr_scores = self.get_predictor_scores(model_name="RandomForest")
        print("PR SCORE", pr_scores)
        ans = {}
        for score in pr_scores:
            ans[score.classifier_name] = score.compare_fold_shap_values(use_all_means=False)
    ...
    def _train_from_mp(cls, bdf, y, groups, classifier_name, classifier_data, scores, params, acc_metrics, mps, use_shap=True, new_pred=None):
        ...
        for train_index, test_index in split:
            ...
            if use_shap is True and best_classifier.__class__.__name__ in COMPATIBLE_MODELS:
                print("Using SHAP!")
                explainer = shap.TreeExplainer(best_classifier)
                # TODO: rerun this w/RF for other classifiers in the fold
                shap_values = explainer.shap_values(X_test)
                if isinstance(shap_values, list):  # If there are multiple classes
            ...
            if hasattr(classifier, 'feature_importances_'):
                feature_importances = grid_search.best_estimator_.named_steps['classifier'].feature_importances_
                # consider saving all of the feature importances
                importance_df = pd.DataFrame({
                    'Feature': X_train.columns,
                    'Importance': feature_importances
                })
