...
class MultiPredictor(LegacyBaseModel):
    ...
    def __init__(self, id=None, task_id=None, sensors=Sensor.where(name=SENSOR_CODES), model=RandomForestClassifier(), items = None, created_at=None, updated_at=None):
        super().__init__()
        self.id = id
    ...
    def gen_items_for_sensor(self, snr, ntaf=True):
        if not ntaf:
            ...
            nfat = Predictor.find_or_create(task_id=self.task_id, sensor_id=snr.id, non_norm=False, abs_val=True)
            nfat.train_it()
            print(f"Generating normed, non abs score, for task: {self.task.description}")
            nfaf = Predictor.find_or_create(task_id=self.task_id, sensor_id=snr.id, non_norm=False, abs_val=False)
            nfaf.train_it()
            print(f"Generating non normed, abs score, for task: {self.task.description}")
            ntat = Predictor.find_or_create(task_id=self.task_id, sensor_id=snr.id, non_norm=True, abs_val=True)
            # ntat.train_it()
            print(f"Generating non normed, non abs score, for task: {self.task.description}")
    ...
    def gen_scores_for_sensor(self):
        for sensor in self.get_sensors():
            predictor = Predictor.find_or_create(task_id=self.task_id, sensor_id=sensor.id, non_norm=True, abs_val=False, multi_predictor_id=self.id)
            predictor = predictor.train_it()
            # Here we assume that the accuracy is stored in predictor.accuracies after training
            print(f"storing {sensor.name}, accuracy: {predictor.accuracies}")
