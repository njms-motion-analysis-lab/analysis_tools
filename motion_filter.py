...
class MotionFilter:
    DURATION: int = 10 # (ms)
    VELOCITY: float = 0.01 # (m/s)
    DISPLACEMENT: float = 0.05 # (cm)

    def __init__(self, exp_sample: ExpMotionSampleTrial, duration: int = DURATION, velocity: float = VELOCITY, displacement: int = DISPLACEMENT) -> None:
    ...
    @classmethod
    def is_valid(cls, sub_motion: list, exp_sample: ExpMotionSampleTrial) -> bool:
    ...
    @classmethod
    def get_valid_motions(cls, exp_sample: ExpMotionSampleTrial) -> list:
        valid_motions = []
