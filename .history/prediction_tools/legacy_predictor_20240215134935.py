...
class Predictor(LegacyBaseModel):
    ...
    def define_classifiers(self, use_shap_compatible=False, y_train=32):
        classifiers = {
            'KNN': {
                'n_neighbors': range(1, max(2, int(np.sqrt(y_train)))),
            ...
            'RandomForest': {
                ...
                'max_features': ['sqrt', 'log2', None] if y_train > 30 else ['sqrt', 'log2']
            },
            'LogisticRegression': {
                'C': np.logspace(-4, 4, 5),
                'penalty': ['l1', 'l2'],
                'solver': ['liblinear']  # 'saga' could be considered if L1 penalty is important for high-dimensional data
            },
            'XGBoost': {
            ...
            'SVM': {
                ...
                'kernel': ['linear', 'rbf']  # 'poly' can be computationally intensive for high dimensions
            },
            'AdaBoost': {
    ...
    @classmethod
    def define_classifiers_cls(cls, use_shap_compatible, use_mini_params, classifier_name=None, width=50):
        if use_mini_params:
