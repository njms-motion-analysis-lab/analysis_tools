...
class MultiPredictor(LegacyBaseModel):
    ...
    def show_predictor_scores(self, models, abs_val=False, non_norm=True, reverse_order=True, first_model_features=None, num_top=NUM_TOP, use_cat=True, axis=False, include_accuracy=False):
        ...
        for model, preds in pred_kit.items():
            if first_model_features is not None:
                first_features = preds[0][0][0].get_top_n_features(NUM_TOP)
            else:
                first_features = None

            for pred in preds:
                ps, acc_score, sensor_obj = pred
                sensor_name = sensor_obj.human_name()
                shap_scores, top_scores, sensors = self.gen_stats([pred], first_model_features=first_features, num_top=NUM_TOP, use_cat=use_cat, axis=axis)
    ...
    def gen_stats(self, preds, first_model_features=None, num_top=NUM_TOP, use_cat=False, axis=False):
        shap_scores = []
        top_scores = []
        sensors = []
        ps = preds[0][0]
        if first_model_features is None:
            top_features = ps[0].get_top_n_features(NUM_TOP)
        else:
        ...
        for ps, acc_score, snr in preds:
            if ps == []:
                continue
            first_shap_scores = ps[0].get_shap_values_for_features(top_features)

            current_shap_features = ps[0].get_top_n_features(NUM_TOP)
            current_shap_scores = ps[0].get_shap_values_for_features(current_shap_features)

            if use_cat is True:
                if axis is True:
                    axis = self.get_new_axis()

                aggregate_shap_values = ps[0].aggregate_shap_values_by_category(current_shap_scores, axis)
                normalized_shap_values = ps[0].normalize_shap_values(aggregate_shap_values)
