...
class Predictor(LegacyBaseModel):
    ...
    def generate_sub_gradient_num(self, sensor=None):
        ...
        for self_pt in self_pts:
            ...
            self_temp, counter_temp = self.get_temp_sub_gradients(self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient, features_dom, features_non_dom)
    ...
    def get_temp_sub_gradients(self, self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient, features_dom=False, features_non_dom=False):
        print("getting temp df abs" if self.abs_val == 1 else "getting temp df norm")

        def get_temp_sub_gradient(patient_pt, patient_sensor, features_loc):
            print("GETTING PT STATS")
            stats_method = abs if self.abs_val == 1 else lambda x: x
            temp_df = pd.DataFrame(
                stats_method(
                    patient_pt.combined_gradient_set_stats_list(patient_sensor, abs_val=self.abs_val, non_normed=self.non_norm, loc=features_loc)['mean']
                )
            ).T
            temp_df.columns = ['grad_data__' + col.split('__')[1] for col in temp_df.columns]
            return temp_df

        self_temp = get_temp_sub_gradient(self_pt, sensor, False)
        counter_temp = get_temp_sub_gradient(counterpart_pt, counterpart_sensor, False)
    ...
    def get_temp_dataframes(self, self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient, features_dom=False, features_non_dom=False):
        ...
        def get_temp_dataframe(patient_pt, patient_sensor, features_loc):
            print("GETTING PT STATS")
            stats_method = abs if self.abs_val == 1 else lambda x: x
            temp_df = pd.DataFrame(
                stats_method(
        ...
        self_temp = get_temp_dataframe(self_pt, sensor, False)
        counter_temp = get_temp_dataframe(counterpart_pt, counterpart_sensor, False)
    ...
    def train_from(self, obj=None, use_shap=False, force_load=False):
        if obj is None:
            obj = self

        bdf, y = self.get_final_bdf(untrimmed=True, force_load=force_load)
        
        
        bdf = Predictor.trim_bdf_with_boruta(bdf, y)
        if len(bdf.columns) <= 2:
            print("Skipping boruta!")
            self.skip_boruta == True
            bdf, y = self.get_final_bdf(force_load=False)

        self.fit_multi_models(bdf, y, use_shap)
        print("Done training!")
    
    def retrain_from(self, use_shap=False, force_load=False):
    ...
    def get_final_bdf(self, force_load=False, force_abs_x=False, untrimmed=False):
        bdf = self.get_df(force_load=force_load)
        y = self.get_y(bdf)
    ...
    def trim_bdf_with_boruta(cls, bdf, y, n_estimators=500, max_depth=5, random_state=42):
        ...
        # Initialize Boruta
        boruta_selector = BorutaPy(rf, n_estimators=1000,random_state=42,verbose=2,max_iter=500,alpha=0.05)
    ...
    def rename_columns_with_orientation(self, bdf):
        ...
        for col in bdf.columns:
            if col not in ['is_dominant', 'patient']:  # Ignore non-feature columns
    ...
    def optimal_silhouette_scores(self, bdf, max_features=250, p_range=False):
    ...
    def get_df(self, force_load=False):
        # relief f (corr based method to reduce feature space)
        # silohuottte (how many features to keep)
        # use this to get right threshold
        # has index to show features to keep
        # davies bouldin
        # start with accuracy
        # add f1
        # let
    ...
    @classmethod
    def evaluate_feature_scores(cls, bdf, max_features=250, min_features=50, min_avg_silhouette_score=0.5):
        columns_to_retain = ['is_dominant', 'patient']
    ...
    def fit_multi_models(self, bdf, y, use_shap=False):
        ...
        if curr_len < MINIMUM_SAMPLE_SIZE:
            ...
            classifiers = self.define_classifiers(y_train=width) 
    ...
    def define_classifiers(self, use_shap_compatible=False, y_train=32):
        classifiers = {
            'KNN': {
                    'classifier': KNeighborsClassifier(),
                    'param_grid': {
                        'classifier__n_neighbors': range(1, max(y_train, 2)),
                        'classifier__weights': ['uniform', 'distance'],
                        'classifier__metric': ['euclidean', 'manhattan', 'minkowski']
                    }
                },
            'RandomForest': {
                'classifier': RandomForestClassifier(),
                'param_grid': {
                    'classifier__n_estimators': [6, 12, 24, 48],
                    'classifier__max_depth': [2, 3, 4, 5],  # Reduced max depth
                    'classifier__min_samples_split': [8, 12, 16],
            ...
            'LogisticRegression': {
                'classifier': LogisticRegression(max_iter=10000, solver='liblinear'),
                'param_grid': {
                    'classifier__C': [0.01, 0.1, 1, 10, 50],  # Extended C range
                    'classifier__penalty': ['l1', 'l2'],
                    'classifier__solver': ['liblinear', 'saga']
                }
            },
            'XGBoost': {
                'classifier': XGBClassifier(),
                'param_grid': {
                    'classifier__learning_rate': [0.005, 0.01, 0.03],
                    'classifier__n_estimators': [50, 75, 100],
                    'classifier__max_depth': [2, 3, 4],  # Slightly increased max depth
                    'classifier__subsample': [0.6, 0.8, 1.0],  # Added subsample
                    'classifier__colsample_bytree': [0.6, 0.8, 1.0]  # Added colsample_bytree
                }
            },
            'CatBoost': {
                'classifier': CatBoostClassifier(verbose=0),
                'param_grid': {
                    ...
                    'classifier__iterations': [200, 250],
                    'classifier__depth': [3, 4, 5]  # Slightly increased depth
                }
            },
            'SVM': {
                    'classifier': SVC(probability=True),
                    'param_grid': {
                        ...
                        'classifier__gamma': [0.01, 0.1, 'scale', 'auto'],
                        'classifier__kernel': ['linear', 'poly']
                    }
                },
            'AdaBoost': {
            ...
            'ExtraTrees': {
                'classifier': ExtraTreesClassifier(),
                'param_grid': {
                    'classifier__n_estimators': [16, 32, 48, 72, 96],  # Decreased n_estimators
                    'classifier__max_depth': [2, 3, 4],
                    'classifier__min_samples_split': [8, 10, 12, 14],  # Increased min samples split
                    'classifier__min_samples_leaf': [4, 5, 6, 8]  # Increased min samples leaf
                }
            },
            'GradientBoosting': {
                'classifier': GradientBoostingClassifier(),
                'param_grid': {
                    'classifier__learning_rate': [0.001, 0.01, 0.03],
                    'classifier__n_estimators': [50, 75, 100],
                    'classifier__max_depth': [2, 3],  # Slightly increased max depth
                }
            },
            'DecisionTree': {
                'classifier': DecisionTreeClassifier(),
                'param_grid': {
                    'classifier__max_depth': range(2, 6),  # Reduced max depth range
                    'classifier__min_samples_split': range(10, 21, 2),
                    'classifier__min_samples_leaf': range(3, 8),
                    'classifier__max_features': ['sqrt', 'log2', None]
                }
            },
        }
        ...
        if use_mini_params:
            ...
            classifiers = {
                'KNN': {
                        'classifier': KNeighborsClassifier(),
                        'param_grid': {
                            'classifier__n_neighbors': range(8, 32),
                            'classifier__weights': ['uniform', 'distance'],
                            'classifier__metric': ['euclidean', 'manhattan', 'minkowski']
                        }
                    },
