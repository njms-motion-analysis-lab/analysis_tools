...
class Predictor(LegacyBaseModel):
    ...
    def generate_sub_gradient_count(self, sensor=None):
        ...
        for self_pt in self_pts:
            ...
            print("YYY")
            print(self_pt, counterpart_pt)
            print("YYY")
            curr_patient = Patient.where(id=self_pt.patient_id)[0].name
            print("ZZZ")
            print(self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient)
            print("ZZZ")
    ...
    def get_temp_sub_gradients_count(self, self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient, features_dom=False, features_non_dom=False):
        def get_temp_sub_gradient_count(patient_pt, patient_sensor, features_loc):
            print("XXXX")
            print(patient_pt.id, patient_sensor.id, features_loc)
            if patient_pt.id == 27 and patient_sensor.id == 49:
                import pdb;pdb.set_trace()
            print("XXX")
            stats_method = abs if self.abs_val == 1 else lambda x: x
            
            temp_df = pd.DataFrame(stats_method(patient_pt.combined_gradient_set_count(patient_sensor, abs_val=self.abs_val, non_normed=self.non_norm, loc=features_loc))).T
            
            


            return temp_df
        print(self_pt, sensor)
