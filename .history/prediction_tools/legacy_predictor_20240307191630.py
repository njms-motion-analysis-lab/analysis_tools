...
class Predictor(LegacyBaseModel):
    ...
    def generate_dataframes(self, sensor=None):
        ...
        # Reset index and ensure dataframes are not empty
        ...
        # Ensure there's no attempt to align columns before concatenation as it caused issues
        if dom_dataframes and nondom_dataframes:
            # Concatenate dom and nondom dataframes directly
            # This assumes both sets already have identical and properly ordered columns
            final_df = pd.concat(dom_dataframes + nondom_dataframes, ignore_index=True)
        else:
