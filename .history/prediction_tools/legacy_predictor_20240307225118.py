...
# Remove this after new data (add_other)
    ...
    def clean_bdf(self, bdf):
        bdf.dropna(axis=1, how='all', inplace=True)  # Drop columns where all values are NaN
        
        # Attempt to encode the 'patient' column
        try:
            # Assuming 'patient' column exists and is intended to be transformed
            bdf['patient'] = label_encoder.fit_transform(bdf['patient'])
        except TypeError:
            ...
            bdf['patient'] = bdf['patient'].str.extract('(\d{2})$').astype(int)
        ...
        bdf.fillna(0, inplace=True)  # Replace any remaining NaN values with 0
        
        # Additional checks and cleaning as previously defined
