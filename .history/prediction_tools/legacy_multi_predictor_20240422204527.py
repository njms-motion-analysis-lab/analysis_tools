...
class MultiPredictor(LegacyBaseModel):
    ...
    def gen_items_for_sensor(self, snr=None, ntaf=True):
        if not ntaf:
            ...
            nfat = Predictor.find_or_create(task_id=self.task_id, sensor_id=snr.id, non_norm=True, abs_val=False, cohort_id=self.cohort_id, multi_predictor_id=self.id)
            nfat.train_from(use_shap=True)
            print(f"Generating normed, non abs score, for task: {self.task().description}")
            nfaf = Predictor.find_or_create(task_id=self.task_id, sensor_id=snr.id, non_norm=True, abs_val=False, cohort_id=self.cohort_id, multi_predictor_id=self.id)
    ...
    def feature_cluster_map(self, non_norm=False, abs_val=abs_val):
