...
class MultiPredictor(LegacyBaseModel):
    ...
    def linkage_mat(self, sim_matrix=None, view=True):
        ...
        if sim_matrix is None:
            sim_matrix, unique_features = self.sim_matrix()
        ...
        if view is True:
            # Plot the dendrogram at different levels of clustering
            for max_d in np.arange(0.1, 1.1, 0.1):  # Adjust the range and step as needed
                plt.figure(figsize=(25, 10))
                plt.title(f'Dendrogram with max_d = {max_d}')
                plt.xlabel('Index Numbers')
                plt.ylabel('Distance')
                dendrogram(Z,labels=unique_features,color_threshold=1.5)
                plt.axhline(y=max_d, c='k', ls='--', lw=0.5)  # Add a horizontal line for the distance threshold
                plt.show()
        else:
