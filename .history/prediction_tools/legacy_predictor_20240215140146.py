...
class Predictor(LegacyBaseModel):
        ...
        classifiers_param_grid = {
            ...
            'RandomForest': {
                ...
                'param_grid': {
                    ...
                    'classifier__max_depth': [None, 10, 20, 30] if feature_count > 50 else [None, 5, 10, 15],
                    ...
                    'classifier__max_features': ['sqrt', 'log2'] if feature_count > 50 else ['auto'],
                }
            },
            'ExtraTrees': {
                'classifier': ExtraTreesClassifier(),
                'param_grid': {
                    ...
                    'classifier__max_depth': [None, 20, 40, 60] if feature_count > 50 else [None, 10, 20, 30],
            ...
            'GradientBoosting': {
                ...
                'param_grid': {
                    ...
                    'classifier__max_depth': [3, 5, 7, 9] if feature_count > 50 else [3, 4, 5],
                }
            },
            'DecisionTree': {
                'classifier': DecisionTreeClassifier(),
                'param_grid': {
                    'classifier__max_depth': list(range(3, 14, 2)) if feature_count > 50 else list(range(2, 10)),
                    'classifier__min_samples_split': list(range(2, 11, 2)),
                    'classifier__min_samples_leaf': list(range(1, 5)),
                    'classifier__max_features': ['sqrt', 'log2', None] if feature_count > 50 else ['auto'],
                }
            },
            'LogisticRegression': {
                'classifier': LogisticRegression(max_iter=10000, solver='liblinear'),
            ...
            'SVM': {
                'classifier': SVC(probability=True),
                'param_grid': {
                    'classifier__C': np.logspace(-3, 2, 6).tolist(),
                    'classifier__gamma': ['scale', 'auto'] + np.logspace(-3, 2, 6).tolist(),
    ...
    @classmethod
    def define_classifiers_cls(cls, use_shap_compatible, use_mini_params, classifier_name=None, y_train=50):
        if use_mini_params:
