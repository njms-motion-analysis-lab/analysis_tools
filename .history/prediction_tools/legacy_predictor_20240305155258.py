...
class Predictor(LegacyBaseModel):
    ...
    def process_alt_tasks(self, self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor):
        dom_dataframes = []
        nondom_dataframes = []
        ...
        for self_pt in self_pts:
            ...
            self_temp = self.get_single_patient_dataframe(self_pt, sensor, curr_patient, features_dom, features_non_dom)
        
        for other_pt in other_pts:
            curr_patient = Patient.get(other_pt.patient_id).name
            cohort = self.cohort().get_alt_cohort()
            self_temp = self.get_single_patient_dataframe(self_pt, sensor, curr_patient, cohort, features_non_dom)
    ...
    def process_patient_tasks(self, self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor):
        if self.cohort().is_alt_compare():
            print("Diff compare!")
            self_temp = self.process_alt_tasks(self_pts, other_pts)
            # If needed, activate the alternative comparison logic here.
            # return self.alt_compare(self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor)
    ...
    def get_temp_dataframe(self, patient_pt, patient_sensor, features_loc):
        ...
        gradient_stats = patient_pt.combined_gradient_set_stats_list(
            patient_sensor, 
            abs_val=self.abs_val, 
            non_normed=self.non_norm, 
            loc=features_loc
        )
