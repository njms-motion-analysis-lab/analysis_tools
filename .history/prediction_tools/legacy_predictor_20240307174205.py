...
class Predictor(LegacyBaseModel):
    ...
    def process_alt_tasks(self, self_pts, other_pts, sensor, nondom_sensor):
        ...
        for self_pt in self_pts:
            print("cohort a length", len(self_pts), "num", num)
            curr_patient = Patient.where(id=self_pt.patient_id)[0].name
        ...
        for other_pt in other_pts:
            print("cohort b length", len(other_pts), "num", num)
            other_patient = Patient.get(other_pt.patient_id).name
            other_cohort = self.cohort().get_alt_cohort()
            other_temp = self.get_single_patient_dataframe(other_pt, nondom_sensor, other_patient, other_cohort, is_dominant=False)
    ...
    def get_temp_dataframe(self, patient_pt, patient_sensor, features_loc):
        ...
        if gradient_stats:  # Check if gradient_stats is not an empty dictionary
            print(gradient_stats)
            if 'mean' in gradient_stats:
                temp_df = pd.DataFrame(stats_method(gradient_stats['mean'])).T
            else:
                # Initialize an empty DataFrame with the expected column names
                temp_df = pd.DataFrame(columns=[f'grad_data__{col}' for col in gradient_stats.keys()])
                print("No 'mean' data available for the given gradient set.")
        else:
            # If gradient_stats is an empty dictionary, return an empty DataFrame
            temp_df = pd.DataFrame()
            print("No data available for the given gradient set.")
    ...
    def get_df(self, force_load=True):
        def gen_df():
            ...
            df_list = []
            for i, sensor_obj in enumerate(same_sensor_objs):
                df = self.generate_dataframes(sensor=sensor_obj)
                df.columns = [f"{col}_{chr(120 + i)}" for col in df.columns]  # 120 is the ASCII code for 'x'
                df_list.append(df)
            import pdb;pdb.set_trace()
            is_dom = df_list[0]['is_dominant']
            pt = df_list[0]['patient']

            columns_to_drop = [f'is_dominant_{chr(120 + i)}' for i in range(3)] + [f'patient_{chr(120 + i)}' for i in range(3)]
            bdf = pd.concat(df_list, axis=1)
        ...
        if self.non_norm == 1:
            if force_load or self.aggregated_stats_non_normed is None:
        ...
        if force_load or self.aggregated_stats is None:
