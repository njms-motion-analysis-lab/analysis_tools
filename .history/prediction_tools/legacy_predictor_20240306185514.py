...
class Predictor(LegacyBaseModel):
    ...
    def process_alt_tasks(self, self_pts, other_pts, sensor, nondom_sensor):
        ...
        for self_pt in self_pts:
            ...
            self_temp = self.get_single_patient_dataframe(self_pt, sensor, curr_patient, cohort, is_dominant=True)
            print(self_temp)
            dom_dataframes.append(self_temp)
        ...
        for other_pt in other_pts:
            ...
            other_temp = self.get_single_patient_dataframe(self_pt, nondom_sensor, other_patient, other_cohort, is_dominant=False)
            print(self_temp)
    ...
    def process_patient_tasks(self, self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor, alt_compare=False):
        # if alt_compare is True:
        #     print("Diff compare!")
    ...
    def generate_dataframes(self, sensor=None):
        ...
        alt_compare = self.is_alt_compare()
        dom_dataframes, nondom_dataframes = self.process_patient_tasks(self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor, alt_compare=alt_compare)
    ...
    def get_single_patient_dataframe(self, self_pt, sensor, curr_patient, cohort, is_dominant=True):
    ...
    def get_temp_dataframes(self, self_pt, counterpart_pt, sensor, counterpart_sensor, curr_patient, features_dom=False, features_non_dom=False):
        ...
        self_temp['cohort'] = cohort.name
        counter_temp['cohort'] = cohort.get_alt_cohort().name
    ...
    def get_df(self, force_load=True):
        def generate_dataframe(sensor_index):
            sensor_obj = same_sensor_objs[sensor_index]
            df = self.generate_dataframes(sensor=sensor_obj)
            df.columns = [f"{col}_{sensor_index}" for col in df.columns]
            return df
        ...
        def gen_df():
            ...
            # Generate dataframes for the x, y, and z sensors
            x_df = generate_dataframe(0)
            y_df = generate_dataframe(1)
            z_df = generate_dataframe(2)
            
            # Extract dominant and patient info from the first dataframe
            is_dom = x_df['is_dominant_0']
            patient_info = x_df['patient_0']
            
            # Combine the dataframes
            combined_df = pd.concat([x_df, y_df, z_df], axis=1)
            combined_df.drop(columns=['is_dominant_0', 'is_dominant_1', 'is_dominant_2', 'patient_0', 'patient_1', 'patient_2'], inplace=True)
            
            # Restore the 'is_dominant' and 'patient' columns
            combined_df['is_dominant'] = is_dom
            combined_df['patient'] = patient_info
            return combined_df

        target_attr = 'aggregated_stats_non_normed' if self.non_norm == 1 else 'aggregated_stats'
        need_update = getattr(self, target_attr) is None or force_load

        if need_update:
            print("Generating aggregated dataframe...")
            aggregated_df = gen_df()
            serialized_stats = memoryview(pickle.dumps(aggregated_df))
            self.update(**{target_attr: serialized_stats})
            
        return pickle.loads(getattr(self, target_attr))
