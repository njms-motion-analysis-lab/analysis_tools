...
class Predictor(LegacyBaseModel):
    ...
    def generate_dataframes(self, sensor=None):
        ...
        return pd.concat([pd.concat(dom_dataframes), pd.concat(nondom_dataframes)])
    ...
    def get_df(self, force_load=False):
        ...
        def gen_df():
            sensor = Sensor.get(self.sensor_id)
            # Determine sensor name and try finding matching 'Y' and 'Z' sensors
            base_name = sensor.name[:-1]
            dimension_indicator = sensor.name[-1]
            same_sensor_objs = [sensor]  # Start with the current sensor
            
            for dim in ['X', 'Y', 'Z']:  # Include 'X' to complete the dimension set
                if dim != dimension_indicator:  # Skip the current sensor's dimension
                    sensor_name = f"{base_name}{dim}"
                    found_sensor = Sensor.find_by(name=sensor_name)
                    if found_sensor:
                        same_sensor_objs.append(found_sensor)
            
            # If not all dimensions found, use the fallback
            if len(same_sensor_objs) < 3:
                same_sensor_objs = Sensor.where(part=sensor.part, side=sensor.side, placement=sensor.placement)
            
            # Ensure we have exactly three sensors for XYZ dimensions
            same_sensor_objs = same_sensor_objs[:3]
            
            # Generate dataframes for each dimension
            df_list = []
            for i, sensor_obj in enumerate(same_sensor_objs):
                df = self.generate_dataframes(sensor=sensor_obj)
                df.columns = [f"{col}_{['x', 'y', 'z'][i]}" for col in df.columns]
                df_list.append(df)
            
            # Concatenate and clean up the dataframe
            combined_df = pd.concat(df_list, axis=1)
            columns_to_drop = ['is_dominant_x', 'is_dominant_y', 'is_dominant_z', 'patient_x', 'patient_y', 'patient_z']
            combined_df.drop(columns=columns_to_drop, inplace=True)
            combined_df['is_dominant'] = df_list[0]['is_dominant_x']
            combined_df['patient'] = df_list[0]['patient_x']
