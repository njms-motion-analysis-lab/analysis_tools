...
class Predictor(LegacyBaseModel):
    ...
    def optimal_silhouette_scores(self, bdf, max_features=500, p_range=False):
    ...
    def define_classifiers(self, use_shap_compatible=False, y_train=32):
        classifiers = {
            ...
            'RandomForest': {
                'classifier': RandomForestClassifier(),
                'param_grid': {
                    'classifier__n_estimators': [2, 4, 6, 12, 24, 48, 72],
                    'classifier__max_depth': [2, 3, 4, 5, 6],  # Reduced max depth
                    'classifier__min_samples_split': [2, 4, 8, 12, 16, 32],
            ...
            'ExtraTrees': {
                'classifier': ExtraTreesClassifier(),
                'param_grid': {
                    'classifier__n_estimators': [4, 8, 16, 32, 48, 72, 96],  # Decreased n_estimators
                    'classifier__max_depth': [2, 3, 4],
                    'classifier__min_samples_split': [8, 10, 12, 14, 28],  # Increased min samples split
                    'classifier__min_samples_leaf': [4, 5, 6, 8, 16]  # Increased min samples leaf
                }
            },
            'GradientBoosting': {
                'classifier': GradientBoostingClassifier(),
                'param_grid': {
                    ...
                    'classifier__max_depth': [2, 3, 6],  # Slightly increased max depth
                }
            },
            'DecisionTree': {
                'classifier': DecisionTreeClassifier(),
                'param_grid': {
                    'classifier__max_depth': range(2, 10),  # Reduced max depth range
        ...
        if use_mini_params:
            ...
            classifiers = {
                'KNN': {
                        'classifier': KNeighborsClassifier(),
                        'param_grid': {
                            'classifier__n_neighbors': range(4, 32),
                            'classifier__weights': ['uniform', 'distance'],
                            'classifier__metric': ['euclidean', 'manhattan', 'minkowski']
                        }
                    },
    ...
    def _train_classifier(self, bdf, y, groups, classifier_name, classifier_data, scores, params, acc_metrics, use_shap=False):
        ...
        for train_index, test_index in split:
            ...
            grid_search = GridSearchCV(pipe, n_jobs=-1, param_grid_classifier, cv=splitter, scoring='accuracy')
