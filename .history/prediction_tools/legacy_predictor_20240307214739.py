...
# Remove this after new data (add_other)
    def get_df(self, force_load=True, add_other=True):
        def gen_df():
            ...
            x["cohort"] = x['cohort_x']
            y["cohort"] = y['cohort_y']
            x["patient"] = x['patient_x']
            y["patient"] = y['patient_y']
            y["is_dominant"] = y['is_dominant_y']
            x["is_dominant"] = x['is_dominant_x']
            x["cohort"] = x['cohort_x']
            y["cohort"] = y['cohort_y']
            ...
            # Step 1: Extract patient Series and convert to lists
            patients_x_list = x['patient_x'].dropna().unique().tolist()
            patients_y_list = y['patient_y'].dropna().unique().tolist()
            patients_z_list = z['patient_z'].dropna().unique().tolist()

            # Step 2: Concatenate lists
            all_patients_list = patients_x_list + patients_y_list + patients_z_list

            # Step 3: Create a DataFrame from the combined list
            patients_master_df = pd.DataFrame(all_patients_list, columns=['patient']).drop_duplicates().dropna()
