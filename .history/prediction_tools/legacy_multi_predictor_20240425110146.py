...
class MultiPredictor(LegacyBaseModel):
    ...
    def linkage_mat(self, sim_matrix=None, view=True, abs_val=False, non_norm=True):
        ...
        if view is True:
            ...
            # Extract the unique colors from the dendrogram
            unique_colors = sorted(set(dendro['color_list']))
            unique_colors = [color for color in unique_colors if color != 'b']

            return label_color_map, unique_colors
