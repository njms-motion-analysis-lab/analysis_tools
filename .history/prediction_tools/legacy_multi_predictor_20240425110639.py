...
class MultiPredictor(LegacyBaseModel):
    ...
    def linkage_mat(self, sim_matrix=None, view=True, abs_val=False, non_norm=True):
        ...
        legend_handles = []
        cluster_colors = {}
        for i, color in enumerate(sorted(set(dendro['color_list']))):
            if color != 'b':  # Exclude the color if it's the default blue used for the last p merges
                ...
                cluster_colors[f"Cluster {i+1}"] = color

        plt.legend(legend_handles, cluster_colors.keys(), loc='upper right')
        ...
        if view is True:
            ...
            return label_color_map, cluster_colors
