...
class Predictor(LegacyBaseModel):
    ...
    def alt_sensor(self, sensor=None, cohort=None):
        # quick way to check cohort we're keeping the sensor adn comparing against task.
    ...
    def generate_dataframes(self, sensor=None):
        ...
        for self_pt in self_pts:
            try:
                ...
                counterpart_pt = counterpart_pts[0]
                curr_patient = Patient.where(id=self_pt.patient_id).first().name
                ...
                # switch places for lefty patient
                if self_pt.patient_id in [21, 26, 27, 28]:
                    print("switching places for lefty pt....")
                    dom_dataframes.append(counter_temp)
                    nondom_dataframes.append(self_temp)
                    print("done....")
