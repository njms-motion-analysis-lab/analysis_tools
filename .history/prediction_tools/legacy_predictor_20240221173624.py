...
class Predictor(LegacyBaseModel):
    ...
    def process_patient_tasks(self, self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor):
        ...
        # Determine which list to treat as 'self_pts' based on length comparison
        swap_pts = len(other_pts) > len(self_pts)
        if swap_pts:
            self_pts, other_pts = other_pts, self_pts
            sensor, nondom_sensor = nondom_sensor, sensor  # Also swap the sensors if swapping pts
