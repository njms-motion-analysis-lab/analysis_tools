...
class MultiPredictor(LegacyBaseModel):
    ...
    def linkage_mat(self, sim_matrix=None, view=True, abs_val=False, non_norm=True):
        ...
        dendro = dendrogram(Z, labels=unique_features, color_threshold=color_threshold, no_plot=True)

        # Count the number of leaves in each cluster
        cluster_sizes = {}
        for color in dendro['color_list']:
            if color != 'C0':  # Exclude the default blue color
                cluster_sizes[color] = cluster_sizes.get(color, 0) + 1

        # Sort the clusters by size in descending order
        sorted_clusters = sorted(cluster_sizes.items(), key=lambda x: x[1], reverse=True)

        # Assign colors to the clusters based on their size
        color_palette = ['red', 'yellow', 'green', 'blue', 'purple', 'orange', 'magenta', 'cyan']
        color_map = {}
        for i, (color, size) in enumerate(sorted_clusters):
            new_color = color_palette[i % len(color_palette)]
            color_map[color] = new_color

        # Update the color list in the dendrogram
        dendro['color_list'] = [color_map.get(color, color) for color in dendro['color_list']]

        # Create the dendrogram plot
        dendrogram(Z, labels=unique_features, color_threshold=color_threshold, leaf_font_size=12)

        # Create a legend for the colors
        legend_handles = []
        for color, size in sorted_clusters:
            legend_handles.append(plt.Line2D([0], [0], color=color_map[color], lw=4))

        plt.legend(legend_handles, [f"Cluster {i+1} (Size: {size})" for i, (_, size) in enumerate(sorted_clusters)], loc='upper right')
