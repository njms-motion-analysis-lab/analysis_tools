...
class Predictor(LegacyBaseModel):
    ...
    def generate_sub_gradient_count(self, sensor=None):
        ...
        if self.cohort().is_alt_compare():
            ...
            other_pts = PatientTask.where(task_id=dom_task.id, cohort_id=compare_cohort.cohort_id)
        else:
    ...
    def generate_dataframes(self, sensor=None):
        ...
        if cohort.is_alt_compare():
            counterpart_sensor = sensor
            compare_cohort = cohort.get_alt_cohort()
            counter_pts = PatientTask.where(task_id=counterpart_task.id, patient_id=self_pt.patient_id, cohort_id=compare_cohort.id)
        else:
            compare_cohort = cohort
            counterpart_sensor = nondom_sensor
        for self_pt in self_pts:
            try:
                counterpart_pts = PatientTask.where(task_id=counterpart_task.id, patient_id=self_pt.patient_id, cohort_id=compare_cohort.id)
