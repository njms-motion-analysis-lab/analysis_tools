...
class Predictor(LegacyBaseModel):
    ...
    def train_from(self, obj=None, use_shap=True, force_load=False, get_sg_count=False):
        if obj is None:
            obj = self
...
# Remove this after new data (add_other)
    ...
    def extract_last_two_columns(self, bdf):
        ...
        if all(col in bdf.columns for col in columns_to_extract):
            self.last_two_columns = bdf[columns_to_extract]
            shortened_bdf = bdf.drop(columns=columns_to_extract)
        else:
