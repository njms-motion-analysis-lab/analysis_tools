...
class MultiPredictor(LegacyBaseModel):
    ...
    def gen_stats(self, model_pred_set, first_model_features=None, num_top=NUM_TOP, use_cat=False, axis=False):
        ...
        for current_model_pred_set, acc_score, snr in model_pred_set:
            ...
            current_shap_scores = current_model_pred_set.get_shap_values_for_features(current_top_features)
            

            current_shap_features = current_model_pred_set[0].get_top_n_features(NUM_TOP)
            current_shap_scores = current_model_pred_set[0].get_shap_values_for_features(current_shap_features)
