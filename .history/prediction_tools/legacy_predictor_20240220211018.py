...
class Predictor(LegacyBaseModel):
    ...
    def generate_dataframes(self, sensor=None):
        ...
        for self_pt in self_pts:
            try:
                if self.cohort_id == 3 or self.cohort_id == 4 or self.cohort_id == 5:
                    counterpart_pts = PatientTask.where(task_id=counterpart_task.id, patient_id=self_pt.patient_id, cohort_id=5)
                else:
