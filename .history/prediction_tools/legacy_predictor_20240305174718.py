...
class Predictor(LegacyBaseModel):
    ...
    def process_alt_tasks(self, self_pts, other_pts, sensor):
        dom_dataframes = []
        nondom_dataframes = []
        ...
        for other_pt in other_pts:
            curr_patient = Patient.get(other_pt.patient_id).name
            cohort = self.cohort().get_alt_cohort()
            other_temp = self.get_single_patient_dataframe(self_pt, sensor, curr_patient, cohort, is_dominant=False)
    ...
    def process_patient_tasks(self, self_pts, other_pts, dom_task, counterpart_task, compare_cohort, sensor, nondom_sensor, alt_compare=False):
        if alt_compare is True:
            print("Diff compare!")
            return self.process_alt_tasks(self_pts, other_pts, sensor)
