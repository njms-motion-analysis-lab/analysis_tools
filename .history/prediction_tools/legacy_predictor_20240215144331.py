        ...
        classifiers = {
            ...
            'RandomForest': {
                ...
                'param_grid': {
                    ...
                    'classifier__n_estimators': [6, 12, 24, 50, 100, 200, 400] if feature_count > 52 else [6, 12, 24, 50],
                    # None allows trees to grow as much as needed, but we limit depth for smaller feature sets
                    'classifier__max_depth': [None, 10, 20, 30] if feature_count > 50 else [None, 2, 4, 6, 8, 10, 16],
            ...
            'ExtraTrees': {
                'classifier': ExtraTreesClassifier(),
                'param_grid': {
                    'classifier__n_estimators': [6, 12, 24, 50, 100, 200, 400] if feature_count > 50 else [6, 12, 24, 50],
                    'classifier__max_depth': [None, 20, 40, 60] if feature_count > 50 else [None, 2, 4, 6, 10, 20, 30],
                    'classifier__min_samples_split': [2, 4, 6, 10, 15],
                    'classifier__min_samples_leaf': [1, 2, 4, 6, 10],
                    'classifier__max_features': ['sqrt', 'log2']
                }
        ...
        if use_mini_params:
            ...
            classifiers = {
                ...
                'RandomForest': {
                    ...
                    'param_grid': {
                        # More trees for more complex models, but start lower for initial runs
                        'classifier__n_estimators': [6, 12, 24, 50, 100, 200, 400],
                        # None allows trees to grow as much as needed, but we limit depth for smaller feature sets
                        'classifier__max_depth': [None, 10, 20, 30] if feature_count > 50 else [None, 2, 4, 10, 16],
                        # Adjusting for the dataset's feature count
                        'classifier__min_samples_split': [2, 5, 10],
                        'classifier__min_samples_leaf': [1, 2, 4],
                        # 'auto' lets the model decide, but specifying can help with large feature sets
                        'classifier__max_features': ['sqrt', 'log2'] if feature_count > 50 else ['log2'],
                    }
                },
                'ExtraTrees': {
                    'classifier': ExtraTreesClassifier(),
                ...
                'DecisionTree': {
                    ...
                    'param_grid': {
                        ...
                        'classifier__min_samples_split': list(range(2, 11, 2)),
                        'classifier__min_samples_leaf': list(range(1, 5)),
                        'classifier__max_features': ['sqrt', 'log2', None] if feature_count > 50 else ['log2'],
                    }
                },
                'LogisticRegression': {
                    'classifier': LogisticRegression(max_iter=10000, solver='liblinear'),
