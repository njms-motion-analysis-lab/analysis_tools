...
class Predictor(LegacyBaseModel):
    ...
    def generate_dataframes(self, sensor=None):
        ...
        dom_dataframes_aligned = [df[standard_column_order] for df in dom_dataframes]
        nondom_dataframes_aligned = [df[standard_column_order] for df in nondom_dataframes if set(df.columns) == set(standard_column_order)]
        final_df = pd.concat(dom_dataframes_aligned + nondom_dataframes_aligned, ignore_index=True)
