...
class Predictor(LegacyBaseModel):
    ...
    def generate_dataframes(self, sensor=None):
        ...
        # Generate dominant and non-dominant data frames
        ...
        # Reset index and drop empty DataFrames
        ...
        # Verify there are DataFrames to concatenate
        if not dom_dataframes and not nondom_dataframes:
            return pd.DataFrame()  # Return an empty DataFrame if there's nothing to concatenate

        # Ensure column order is consistent across all DataFrames
        # Using the first DataFrame's columns as the standard order
        standard_column_order = dom_dataframes[0].columns.tolist()

        # Align DataFrames to the standard column order
        aligned_dom = [df[standard_column_order] for df in dom_dataframes]
        aligned_nondom = [df[standard_column_order] for df in nondom_dataframes]

        # Concatenate the aligned DataFrames
        final_df = pd.concat(aligned_dom + aligned_nondom, ignore_index=True)
