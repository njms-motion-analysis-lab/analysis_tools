...
class MultiPredictor(LegacyBaseModel):
    ...
    def aggregate_shap_values(self, models=TOP_TREE_MODELS, abs_val=False, non_norm=True, reverse_order=True, first_model_features=None, num_top=NUM_TOP, use_cat=True, axis=False, include_accuracy=False):
        ...
        for m in models:
            ...
            pred_sensor_kit[m] = {}
            for sensor_score in pred_kit[m]:
                aggregated_shap_values_retrieved, combined_X_np, combined_X_np_cols = sensor_score[0][0].get_matrix("matrix")
                
                # Perform clustering for each sensor and model
                exp = sensor_score[0][0].cluster_features_shap()
                print(exp)
                sensor_object = sensor_score[-1]  # Assuming this is the sensor object
                pred_sensor_kit[m][sensor_object] = exp

        return pred_sensor_kit

    def feature_cluster_map(self, non_norm=False):
        sensor_output = self.aggregate_shap_values(non_norm=False)
        feature_cluster_map = {}

        for model, sensors in sensor_output.items():
            for sensor, features in sensors.items():
                for feature, cluster in features:
                    if feature not in feature_cluster_map:
                        feature_cluster_map[feature] = []
                    feature_cluster_map[feature].append(cluster)
        
        return feature_cluster_map
    
    def sim_matrix(self, feature_cluster_map=None):
        if feature_cluster_map is None:
            feature_cluster_map = self.feature_cluster_map(non_norm=False)
        ...
        for i, feature_i in enumerate(unique_features):
            for j, feature_j in enumerate(unique_features):
                ...
                similarity = sum(1 for x, y in zip(feature_cluster_map[feature_i], feature_cluster_map[feature_j]) if x == y)
                # Normalize by the number of models/sensors they were both part of
                total = len(feature_cluster_map[feature_i])
                normalized_similarity = similarity / total if total > 0 else 0
    ...
    def linkage_mat(self, sim_matrix=None, view=True):
        ...
        # Determine clusters from the hierarchical clustering
        feature_cluster_map = self.feature_cluster_map(non_norm=False)
        unique_features = list(feature_cluster_map.keys())
        max_d = 0.5  # You may need to adjust this based on your dendrogram
        clusters = fcluster(Z, max_d, criterion='distance')
        dendro = dendrogram(Z, labels=unique_features)
        if view is True:
            # Plot the dendrogram
            plt.show()
        else:
    ...
    def show_predictor_scores(self, models, abs_val=False, non_norm=True, reverse_order=True, first_model_features=None, num_top=NUM_TOP, use_cat=True, axis=False, include_accuracy=False):
        ...
        for model, model_pred_sets in pred_kit.items():
            ...
            for model_pred_set in model_pred_sets:
                ...
                shap_scores, top_scores, sensors = self.gen_stats([model_pred_set], first_model_features=first_features, num_top=NUM_TOP, use_cat=use_cat, axis=axis)
    ...
    def get_new_axis(self):
        cluster_assignments = self.linkage_mat(view=False)
    ...
    def gen_stats(self, model_pred_set, first_model_features=None, num_top=NUM_TOP, use_cat=False, axis=False):
        shap_scores = []
        top_scores = []
        sensors = []
        ...
        if first_model_features is None:
            ...
            top_features = first_model_features
    
        for predictor_score, acc_score, snr in model_pred_set:
            if predictor_score == []:
                print("PredictorScore not found!")
                continue
            first_shap_scores = predictor_score[0].get_shap_values_for_features(top_features)
            current_shap_scores = predictor_score.get_shap_values_for_features(top_features)

            current_shap_features = predictor_score[0].get_top_n_features(NUM_TOP)
            current_shap_scores = predictor_score[0].get_shap_values_for_features(current_shap_features)

            if use_cat is True:
                if axis is True:
                    axis = self.get_new_axis()

                aggregate_shap_values = predictor_score[0].aggregate_shap_values_by_category(current_shap_scores, axis)
                normalized_shap_values = predictor_score[0].normalize_shap_values(aggregate_shap_values)
