...
class DynamicPositionSet(LegacyBaseModel):
    ...
    def add_sensor(self, sensor):
        if self.sensor_id == sensor.id:
            print("This DynamicPositionSet is already associated with the provided sensor.")
            return

        self.sensor_id = sensor.id
        self.update(sensor_id=self.sensor_id)
        print(f"Sensor with ID {sensor.id} has been associated with this DynamicPositionSet.")


    def get_patient_task_id(self):
        self._cursor.execute("SELECT patient_task_id FROM dynamic_position_set WHERE id=?", (self.id,))
