...
class OldGenerator:
    ...
    def set_analysis_me_attributes(self):
        ...
        # Extract cohort (e.g., "analysis_me_group_1")
        self.cohort = f"{parts[1]}_{parts[2].replace(' ', '_')}"
        
        # Extract patient name (e.g., "amg1_S019")
        patient_code = parts[3].upper()
        cohort_code = self.cohort[11:12]  # Extracts the group number
        self.patient_name = f"amg{cohort_code}_{patient_code}"
        
        # Extract task and trial
        task_part = parts[5].split('_')[0]  # Extract task name and make it title case
        task_number = parts[5].split('_')[1][-1]  # Extract task number
        self.task = f"{task_part.title()}{task_number}"
        
        trial_part = parts[5].split('_')[2]
        self.trial = int(re.search(r'\d+', trial_part).group())  # Extracts the trial number using regex
