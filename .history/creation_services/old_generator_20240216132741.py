    ...
    def generate_models(self):
        ...
        if pm == None:
            ...
            for key, value in self.data.items():
                ...
                Trial.find_or_create(name=self.task, patient_task_id=pm.id, trial_num=self.trial)
    ...
    def calculate_acceleration_gradients(self, data):
        ...
        for name in data.dtype.names:
            ...
            if "Accelerometer" in name:
                ...
                # Note: np.gradient returns the gradient as a NumPy array
                gradient = np.gradient(data[name], edge_order=2)
                print("yo")
                
                # Store the gradient in the dictionary
                gradient_key = f"gradient_{name}"
                gradient_data[gradient_key] = gradient
        
        # Assuming you want to add this directly to 'self' for access like self.data['gradients']
        # This step may need to be adjusted based on how you intend to use 'gradient_data'
        self.gradients = gradient_data

        # For demonstration, let's also return gradient_data
