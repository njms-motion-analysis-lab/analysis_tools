...
class OldGenerator:
    ...
    def calculate_velocity_from_acceleration_with_timestamps(self, data):
        ...
        extracted from the timestamp data.

        Parameters:
        - data (np.ndarray): Structured array with acceleration and timestamp data.

        Returns:
        - dict: A dictionary with keys for each acceleration data column and their integrated velocity data.
        """
        velocity_data = {}
        
        # Extract time intervals from the "TimeStamp(epoch)" column
        timestamps = data["TimeStamp(epoch)"]
        # Calculate time intervals in seconds (assuming the timestamps are in milliseconds)
        time_intervals = np.diff(timestamps) / 1000.0  # Now this will have one less element than original data
        
        # Iterate through each column name in the structured array
        for name in data.dtype.names:
            if "Accelerometer" in name:
                # Use np.diff to align acceleration array length with time_intervals
                acceleration_diff = np.diff(data[name])
                # Perform numerical integration on the acceleration data to get velocity
                # Integrate acceleration differences with time intervals
                velocity = np.cumsum(acceleration_diff * time_intervals)
                # Insert an initial velocity at the beginning to align with original data length
                # Assuming initial velocity is 0
                velocity = np.insert(velocity, 0, 0)
                
                # Prepare a new key for the velocity data
                velocity_key = f"velocity_{name}"
                
                # Store the integrated velocity in the dictionary
                velocity_data[velocity_key] = velocity
        
        # Assuming you want to add this directly to 'self' for access
        self.velocities = velocity_data

        # For demonstration, let's also return velocity_data
