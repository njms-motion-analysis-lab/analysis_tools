...
class OldGenerator:
    ...
    def generate_models(self):
        ...
        if pm == None:
            ...
            gradient_data = self.calculate_acceleration_gradients(self.data)
    ...
    def calculate_velocity_from_acceleration_with_timestamps(self, data):
        ...
        # Append a 0 at the beginning to maintain the array length
        time_intervals = np.insert(time_intervals, 0, 0)
        ...
        for name in data.dtype.names:
            if "Accelerometer" in name:
                ...
                # Use the calculated time intervals for integration
                acceleration = data[name]
                # Prepend a 0 to acceleration to align with the length of time_intervals after np.diff
                velocity = np.cumsum(np.insert(acceleration, 0, 0) * time_intervals)
