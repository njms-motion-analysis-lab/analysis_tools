...
class OldGenerator:
    ...
    def set_analysis_me_attributes(self):
        ...
        # Ensure the list 'parts' has enough elements
        if len(parts) < 6:
            print("Error: Unexpected file path structure.")
            return

        # Extract patient name
        print(parts)
        patient_code = parts[3].upper()
        cohort_code = self.cohort.name[11:12]  # Assuming 'cohort' has an attribute 'name'
        self.patient_name = f"amg{cohort_code}_{patient_code}"

        # Extract task and trial, adjusting for different path structures
        task_trial_part = parts[5].split('_')
        if len(task_trial_part) >= 3:  # Check if there are enough parts to split
            task_part = task_trial_part[0]
            task_number = task_trial_part[1][-1]
            self.task = f"{task_part.title()}{task_number}"
            trial_part = task_trial_part[2]
            self.trial = int(re.search(r'\d+', trial_part).group())  # Extracts the trial number
        else:
            print("Error: Task and trial information is missing or not in the expected format.")
