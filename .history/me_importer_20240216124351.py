...
if __name__ == '__main__':
    ...
    # Example usage:
    RAW_DATA_FOLDER = 'path/to/your/raw/data/folder'  # Update this with the path to your raw data folder
    files_to_delete = []
    ...
    for subdir, _, filenames in os.walk(RAW_DATA_FOLDER):
        for filename in filenames:
            ...
            # Check if the file is a CSV and convert it
            if file_path.lower().endswith('.csv'):
                convert_csv_and_preserve_headers(file_path)
            elif file_path.lower().endswith('.npy'):
                # Instead of appending, directly delete or mark for deletion
                files_to_delete.append(file_path)

    # Optionally, delete marked NPY files here if needed
    for npy_file in files_to_delete:
        os.remove(npy_file)
