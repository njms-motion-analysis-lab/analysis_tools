...
class MatrixPlotter(LegacyBaseModel):
    ...
    def view_and_save_results(results, results_two=None, task=None, h_one=None, h_two=None, alt=False):
        ...
        figsize = (20, 10)  # Adjust the figure size as needed
        # Plotting
        if df2 is not None:
            # Set 'figsize' to ensure both subplots have the same width and appropriate height
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(24, len(df1)/2))  # The height is set dynamically based on the number of rows
            vmin = min(df1.min().min(), df2.min().min())
            vmax = max(df1.max().max(), df2.max().max())
        else:
            fig, ax1 = plt.subplots(figsize=(12, len(df1)))  # Only one heatmap, set the size accordingly
            vmin, vmax = df1.min().min(), df1.max().max()

        sns.heatmap(df1.astype(float), cmap="YlGnBu", annot=True, vmin=vmin, vmax=vmax, ax=ax1, cbar=False)
