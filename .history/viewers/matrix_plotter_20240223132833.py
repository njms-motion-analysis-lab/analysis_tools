...
class MatrixPlotter(LegacyBaseModel):
    ...
    def view_and_save_results(results, results_two=None, task=None, h_one=None, h_two=None, alt=False):
        ...
        # Use 'aspect' to ensure cells are square
        sns.heatmap(df1.astype(float), cmap="YlGnBu", annot=True, vmin=vmin, vmax=vmax, ax=ax1, cbar=df2 is None, aspect='equal')
        ax1.set_title(h_one if h_one else "Heatmap 1")
        ax1.set_xticklabels(ax1.get_xticklabels(), rotation=45, ha='right')
        ax1.set_yticklabels(ax1.get_yticklabels(), rotation=0)

        if df2 is not None:
            sns.heatmap(df2.astype(float), cmap="YlGnBu", annot=True, vmin=vmin, vmax=vmax, ax=ax2, cbar=True, aspect='equal')
