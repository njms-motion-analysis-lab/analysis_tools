...
class MultiPlotter:
    ...
    def convert_feature_name(self, feature_name):
        ...
        # Split the name into parts for processing
        parts = feature_name.split('_')

        # Dictionary for translating key words
        translations = {
            "second": "jerk",
            "first": "lateness",
            "location": "velocity",
            ...
            "below": "below mean",
            "skewness": "asymmetry",
            "variation": "standard deviation to mean ratio",
            "coefficen": "coefficient"
        }

        # Translate and construct the new feature name
        human_readable = []
        axis = ''
        for part in parts:
            if part in translations:
                human_readable.append(translations[part])
            elif part in ['x', 'y', 'z']:
                axis = part
            else:
                human_readable.append(part)

        # Construct the final human-readable name
        human_readable_name = "—".join(filter(None, human_readable))
        if axis:
            human_readable_name += f"—{axis}-axis"

        return human_readable_name

    def plot_feature_to_accuracy(self, models, multi_predictor):
        # from each mp
        # get predictor accuracies for models and weights
        #  
        # sensors_acc = {''}
