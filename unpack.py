...
def get_params_for_column(column_name):
    ...
    # Get the rest of the column name
    ...
    if rest_of_name.startswith(prefix):
        rest_of_name = rest_of_name[len(prefix):]

    # Check and remove suffixes
    for suffix in suffixes:
        if rest_of_name.endswith(suffix):
            rest_of_name = rest_of_name[:-len(suffix)]
            break

    # Split the remaining name into parts
    name_parts = rest_of_name.split('_')
    if len(name_parts) < 3:
        return None  # Invalid column name format

    # Extract feature name, feature index, and axis
    try:
        feature_index = int(name_parts[-2])  # The second last part is the index
        axis = name_parts[-1]  # The last part is the axis
        feature_name = '_'.join(name_parts[:-2])  # Join all parts except the last two
    except ValueError as e:
        print(f"Error parsing column name: {rest_of_name}. Details: {e}")
    ...
    if isinstance(feature_params, list):
        if feature_index < len(feature_params):
            params = feature_params[feature_index]
        else:
            return None  # Index out of range for the feature parameters
    else:
        params = feature_params  # For features with a single parameter set or no parameters
