...
class ScoliosisTimePredictor:
    ...
    def grid_search_pipeline(
        ...
        """
        Performs GridSearchCV over classification models.
        Returns the best estimator, its metrics, and its name.
        Also attempts to compute and display SHAP values.
        
        Additionally, before grid search, it refines the training features using
        ...
        # Use a RandomForestClassifier for Boruta (since target is binary)
        ...
        # --- Proceed with Grid Search using the refined features ---
        best_estimator = None
        best_score = -np.inf
        ...
        try:
            ...
            top_indices = np.argsort(abs_mean_shap)[-25:]
            X_test_top = X_test.iloc[:, top_indices]
            shap_values_top = shap_values[:, top_indices]

            print("Displaying SHAP beeswarm plot for top 25 features...")
            shap.summary_plot(shap_values_top, X_test_top, max_display=25, plot_type="dot")
        except Exception as e:
            print(f"Could not compute or display SHAP values: {e}")
